...
public class JwtService : IJwtService
{
    ...
    public async Task<(string Token, LoginResponse Response)?> Authenticate(LoginRequest request)
    {
        if (
        ...
        {
            throw new ApplicationException("Brugernavn og adgangskode skal udfyldes.");
        }
        ...
        if (user == null)
        {
            throw new UnauthorizedAccessException("Ugyldigt brugernavn eller adgangskode"); // wrong username
        }
        ...
        {
            throw new UnauthorizedAccessException("Ugyldigt brugernavn eller adgangskode"); // Wrong password
        }

        if (request.SelectedRole.HasValue && !user.Roles.HasFlag(request.SelectedRole.Value))
        {
            throw new UnauthorizedAccessException(
                $"{user.Username} har ikke rettigheder til den valgte rolle."
            );
        }

        var accessToken = CreateAccessTokenWithSpecificRole(
            user,
            request.SelectedRole,
            out DateTime expiryTime,
            out string activeRoleString
        );
        var refreshToken = Guid.NewGuid().ToString("N") + Guid.NewGuid().ToString("N"); // letters and numbers only

        user.RefreshToken = refreshToken;
        user.RefreshTokenExpiryTime = DateTime.UtcNow.AddDays(7); // if no activity from customer in 7 days, logs the user out
    ...
    public async Task<(string Token, LoginResponse Response)?> RefreshToken(RefreshRequest request)
    {
        ...
        {
            throw new SecurityTokenException("Ugyldigt eller udløbet refresh token");
        }
    ...
    public async Task<(string Token, LoginResponse Response)?> SwitchRole(
        Guid userId,
    ...
    )
    {
        ...
        bool isComposite =
            Enum.GetValues(typeof(Role))
                .Cast<Role>()
                .Count(r => r != Role.None && request.TargetRole.HasFlag(r)) > 1;

        var accessToken = CreateAccessTokenWithSpecificRole(
            user,
            isComposite ? null : request.TargetRole,
            out DateTime expiryTime,
            out string activeRoleString
        );
    ...
    private string CreateAccessTokenWithSpecificRole(
        trackrv2_efc.Entities.User user,
        Role? selectedRole,
        out DateTime expiryTime,
        out string activeRoleString
    )
